...
var NotResumable = function(opts){
    ...
    $.assignBrowse = function(domNodes) {
        ...
        $h.each(domNodes, function(domNode) {
            ...
            $h.addEvent(input, 'change', function changeEvent(e) {
                var newClone = input.cloneNode();
                appendFilesFromDomElements([input], e);
                //remove event
                $h.removeEvent(input, 'change', changeEvent);
                // change current input with new one
                input.parentNode.replaceChild(newClone, input);
                input = newClone;
                // reset new input
                input.value = '';
                $h.addEvent(input, 'change', changeEvent);
            });
        });
    };
    ...
    $.pause = function() {
        // not supported, maybe for files queue or cancel current
    };
    $.cancel = function(){
        // TODO
    };
    $.progress = function(){
        // TODO totalFilesUploaded/totalFiles
//        return(totalSize>0 ? totalDone/totalSize : 0);
    };
    $.addFile = function(file) {
        // is domElement ?
        if (file instanceof HTMLInputElement) {
            appendFilesFromDomElements([file]);
        }
    };
